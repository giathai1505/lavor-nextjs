...
const DetailProduct: React.FC<IDetailProduct> = ({
  ...
  const [activeVariant, setActiveVariant] = useState<TImageItem>(renderListImage(product)[0])
const [productImages, setProductImages] = useState<TImageItem[]>(renderListImage(product))
    ...
    <div className="bg-black p-5 md:p-10 xl:px-0 xl:py-16">
      <div className="wrapper">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <DetailImage productImages={renderListImage(product)} activeVariant={activeVariant} onChangeVariant={setActiveVariant}/>
          <DetailContent product={product} productImages={renderListImage(product)}   activeVariant={activeVariant} onChangeColor={handleChangeColor}/>
        </div>
